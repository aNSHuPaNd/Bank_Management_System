...
public class FastCash extends JFrame implements ActionListener {
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == back) {
            ...
            try {
                Conn c = new Conn();
                ResultSet rs = c.s.executeQuery("select * from bank where pinNo = '" + pinNumb + "'");
                int balance = 0;
                while (rs.next()) {
                ...
                Date date = new Date();
                String query = "insert into bank values('"+pinNumb+"', '"+date+"', 'Withdrawl', '"+amount+"')";
